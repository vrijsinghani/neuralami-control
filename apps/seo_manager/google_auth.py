...
from apps.seo_manager.oauth_utils import (
    get_credentials,
    get_analytics_service as get_analytics_service_util,
    get_search_console_service as get_search_console_service_util,
    get_service_account_credentials
)

# Import OAuthManager for scope definitions
...
def get_google_auth_flow(request):
    ...
    """
    # Use the OAuthManager to create the flow with centralized scope definitions
...
def get_analytics_accounts_service_account(service_account_json):
    ...
    try:
        # Use centralized utility to create service account credentials
...
def get_search_console_service(credentials):
    # Use centralized utility to create Search Console service
...
def get_search_console_properties(credentials_or_json):
    ...
    try:
        if isinstance(credentials_or_json, str):
            # Use centralized utility to create service account credentials
            credentials = get_service_account_credentials(
                service_account_json=credentials_or_json,
                service_type='sc'
            )
        else:
        ...
        # Use centralized utility to create Search Console service
        service = get_search_console_service_util(credentials)
