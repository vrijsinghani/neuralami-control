...
class MetaTagsTaskConsumer(OrganizationAwareConsumer):
    ...
    async def progress_update(self, event):
        ...
        try:
            ...
            # Format message for HTMX WebSocket extension
            # The ws-message="progress" element will handle this
            message = {
                'progress': json.dumps(event['progress'])
            }
    ...
    async def status_update(self, event):
        ...
        try:
            ...
            # Format message for HTMX WebSocket extension
            # The ws-message="status" element will handle this
            message = {
                'status': json.dumps(data)
            }

            logger.debug(f"Sending status message to WebSocket: {message}")
            await self.send(text_data=json.dumps(message))
        except Exception as e:
